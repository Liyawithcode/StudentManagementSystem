...
export const sendVerificationOtp = async (email, otp, userName = "Unverified User") => {
    ...
    const html = `
        <div style="max-width: 600px; margin: 0 auto; padding: 30px; font-family: system-ui, -apple-system, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
            ...
            <p style="font-size: 16px; color: #334155; line-height: 1.6; margin-bottom: 24px;">
                Thank you for registering at Student Management System. Use the following 6-digit verification code to complete your signup process:
            </p>
            ...
            <div style="border-top: 1px solid #e2e8f0; margin-top: 30px; padding-top: 20px; text-align: center;">
                <span style="font-size: 13px; color: #94a3b8; font-weight: 500;">Student Management System</span>
            </div>
        </div>
    `;
    ...
    try {
        ...
        console.warn("\n==============================================");
        console.warn(`[OTP LOG] Email sending failed: ${error.message}`);
        console.warn(`[OTP LOG] Verification OTP for ${email} is: ${otp}`);
        console.warn("==============================================\n");
...
export const sendPasswordResetOtp = async (email, otp, userName = "User") => {
    ...
    const html = `
        <div style="max-width: 600px; margin: 0 auto; padding: 30px; font-family: system-ui, -apple-system, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
            <h2 style="color: #4f46e5; font-size: 24px; font-weight: 700; margin-top: 0; margin-bottom: 24px;">Reset Your Password</h2>
            <p style="font-size: 16px; color: #1e293b; line-height: 1.5; margin-bottom: 12px;">Hello ${userName},</p>
            <p style="font-size: 16px; color: #334155; line-height: 1.6; margin-bottom: 24px;">
                We received a request to reset your password. Use the following 6-digit verification code to complete the process:
            </p>
            <div style="text-align: center; margin: 30px 0;">
                <div style="display: inline-block; background-color: #f1f5f9; padding: 16px 40px; border-radius: 12px; font-size: 32px; font-weight: 700; color: #4f46e5; letter-spacing: 4px; border: 1px solid #e2e8f0;">
                    ${otp}
    ...
    try {
        ...
        console.warn("\n==============================================");
        console.warn(`[OTP LOG] Email sending failed: ${error.message}`);
        console.warn(`[OTP LOG] Password Reset OTP for ${email} is: ${otp}`);
        console.warn("==============================================\n");
        return { success: false, error: error.message };
    }
};
